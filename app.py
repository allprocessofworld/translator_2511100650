...
TARGET_LANGUAGES = OrderedDict({
    ...
    # [영어권 커스텀 순서]
    "en-IE": {"name": "영어 (아일랜드)", "code": "EN-GB", "is_beta": False},
    "en-GB": {"name": "영어 (영국)", "code": "EN-GB", "is_beta": False},
    "en-AU": {"name": "영어 (호주)", "code": "EN-AU", "is_beta": False},
    "en-IN": {"name": "영어 (인도)", "code": "EN-GB", "is_beta": False},
    ...
    "fil": {"name": "필리핀어", "code": "FIL", "is_beta": False},
...
if st.session_state.video_details:
    ...
    if st.button("2. 전체 언어 번역 실행"):
        ...
        for i, (ui_key, lang_data) in enumerate(TARGET_LANGUAGES.items()):
            ...
            if title_err or desc_err:
                st.warning(f"DeepL 실패 ({lang_name}). Google 번역으로 대체합니다.")
                ...
                if title_err_g or desc_err_g:
                    result_data["api"] = "Google"; result_data["status"] = "실패"
                    result_data["title"] = f"오류: {title_err_g}"; result_data["desc"] = f"오류: {desc_err_g}"
                else:
                    result_data["api"] = "Google"; result_data["status"] = "성공"
                    result_data["title"] = title_text_g; result_data["desc"] = desc_text_g
            else:
                result_data["api"] = "DeepL"; result_data["status"] = "성공"
    ...
    if st.session_state.translation_results:
        ...
        for result_data in st.session_state.translation_results:
            ...
            final_data_entry = {
                "Language": lang_name, "UI_Key": ui_key, "Title": result_data["title"],
                "Description": result_data["desc"], "Engine": result_data["api"], "Status": status
            }

            with cols[col_index]:
                with st.expander(f"**{lang_name}** (검수)", expanded=False):
                    if status == "성공": st.caption(f"번역 엔진: {result_data['api']}")
                    else: st.caption(f"번역 엔진: {result_data['api']} (실패)")
                ...
                with st.spinner("한국어 ➡ 영어 번역 진행 중... (1차 번역 + 역번역 검수)"):
                    ...
                    try:
                        for i in range(0, len(texts_to_translate_ko), CHUNK_SIZE):
                            ...
                            if translate_err:
                                st.warning(f"KO->EN DeepL 실패 (Chunk {i}). Google 대체.")
                    ...
                    except Exception as e: st.error(f"KO->EN SBV 생성 실패: {str(e)}")
...
if 'sbv_ko_to_en_result' in st.session_state and st.session_state.sbv_ko_to_en_result:
    st.download_button("✅ 번역된 영어 .sbv 파일 다운로드", st.session_state.sbv_ko_to_en_result.encode('utf-8'), "translated_en.sbv", "text/plain")
        ...
        if parse_err: st.error(parse_err)
            ...
            if st.button("SBV 파일 번역 실행"):
                ...
                for i, (ui_key, lang_data) in enumerate(TARGET_LANGUAGES.items()):
                    ...
                    except Exception as e: st.session_state.sbv_errors.append(f"SBV 실패 ({lang_name}): {str(e)}")
        ...
        if parse_err: st.error(parse_err)
            ...
            if st.button("SRT 파일 번역 실행"):
                ...
                for i, (ui_key, lang_data) in enumerate(TARGET_LANGUAGES.items()):
                    ...
                    except Exception as e: st.session_state.srt_errors.append(f"SRT 실패 ({lang_name}): {str(e)}")
